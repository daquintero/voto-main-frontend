...
export const getRelatedIndividuals = (subsetNumber, parentClass, parentId) => (dispatch) => {
  dispatch({
    type: GET_RELATED_INDIVIDUALS.REQUEST,
  });
  return individualService.get.relatedIndividuals(subsetNumber, parentClass, parentId).then(
    response => dispatch({
