...
export const getRelatedFinanceItems = (subsetNumber, parentClass, parentId) => (dispatch) => {
  dispatch({
    type: GET_RELATED_FINANCE_ITEMS.REQUEST,
  });
  return financeItemService.get.relatedFinanceItems(subsetNumber, parentClass, parentId).then(
    response => dispatch({
