...
export const getRelatedOrganizations = (subsetNumber, parentClass, parentId) => (dispatch) => {
  dispatch({
    type: GET_RELATED_ORGANIZATIONS.REQUEST,
  });
  return organizationService.get.relatedOrganizations(subsetNumber, parentClass, parentId).then(
    response => dispatch({
